...
import { generateChapters, type ChapterItem } from "@/lib/media/chapters";
import { getTranscriptByWorkspaceId, getWorkspaceByPublicId } from "@/models/workspace";
...
/**
 * POST /api/media/chapters
 * Body: { workspaceId } or { title?, durationSeconds?, transcript: Segment[] }
 */
export async function POST(req: NextRequest) {
  try {
    ...
    if (!segments.length) {
      return respErr("Transcript required to generate chapters");
    }

    const chapters: ChapterItem[] = await generateChapters({
      title,
      durationSeconds,
      segments,
    });

    return respData({ chapters });
  } catch (e) {
    console.error("chapters generate failed:", e);
    return respErr(e instanceof Error ? e.message : "Failed to generate chapters");
  }
}
